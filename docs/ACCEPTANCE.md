...
Odbiór assetów 3D jest teraz zautomatyzowany w `Scripts/Build-Windows.ps1` i `Scripts/Build-Geography.ps1`.

Przed packagingiem wymagane są rzeczywiste raporty z UE 5.8:

- [ ] `Saved/RuntimeAssetQA/model_quality.json` — PASS: skala źródłowa/bounds, LOD, simple collision, materiały, skeleton i Physics Asset bazowych postaci.
- [ ] `Saved/RuntimeAssetQA/retarget.json` — PASS: IK Rig, wymagane chains, Full Body IK, retarget wszystkich semantic animations na męski i żeński target Quaternius.
- [ ] `Saved/RuntimeAssetQA/scene.json` — PASS: transformy i visual meshes wygenerowanej kampanii.
- [ ] `Saved/RuntimeAssetQA/geography_scene.json` — PASS dla builda GIS.
- [ ] `Saved/RuntimeAssetQA/character_screenshots/capture.json` — PASS: sześć krytycznych póz w 1920×1080, wariant męski i żeński.
...
Runtime nie używa już bryłowych cube-proxy dla głównych aktorów gameplayowych. Modele Quaternius/Kenney/Poly Haven/OpenGameArt są powiązane z kampanią i GIS, ale nie oznacza to finalnego art passu. Dokumenty, fotografie, monitoring i finał nadal przekazują część informacji tekstowo; nie ma finalnych fotografii, nagranego materiału CCTV, dialogów głosowych ani filmowej sceny finałowej. Wizjer korzysta z rzeczywistej kamery korytarza. Audio jest syntezowane. Źródłowe animacje CC0 są zmapowane, a pipeline automatyzuje IK Rig/FBIK/retarget na docelowe szkielety Quaternius; rzeczywisty wynik musi zostać zaliczony przez raporty Runtime Asset QA i visual review w UE 5.8.
