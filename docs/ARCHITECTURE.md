...
`ADriveableVehicle` jest prototypem fizycznego pawna z czterema promieniami podparcia. Generator GIS podłącza audytowane CC0 body i cztery osobne koła z `Data/model_bindings.json`; runtime nie renderuje już kostkowego nadwozia. Nadal nie jest to implementacja Chaos Wheeled Vehicle ani finalny model fizyki opon. `URaceSession` używa wspólnego `RaceProgress`, trzech Data Assets i osobnego slotu rekordów. Nie ma jeszcze powiązania nagród wyścigów z kampanią ani AI kierowców. Próby nie udają nieistniejących przeciwników.
