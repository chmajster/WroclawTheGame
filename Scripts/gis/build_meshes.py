...
def generate(destination):
    data,geo=build(); cells={};size=12800
    def triangle(kind,points):
        center=[sum(p[i] for p in points)/3 for i in range(3)]
        key=(kind,math.floor(center[0]/size),math.floor(center[1]/size))
        record=cells.setdefault(key,{'kind':kind,'origin':[key[1]*size,key[2]*size,0],'vertices':[],'triangles':[]})
    ...
    west,south,east,north=data['bbox'];columns=128;rows=128
    for y in range(rows):
        for x in range(columns):
            face('terrain',[geo.world(west+(east-west)*u/columns,north-(north-south)*v/rows) for u,v in ((x,y),(x+1,y),(x+1,y+1),(x,y+1))])
    destination.mkdir(parents=True,exist_ok=True)
    meshes=[]
    for (kind,x,y),record in sorted(cells.items()):
        filename=f'{kind}_{x}_{y}.json';(destination/filename).write_text(json.dumps(record,separators=(',',':')))
