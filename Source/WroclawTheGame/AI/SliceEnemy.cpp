...
ASliceEnemy::ASliceEnemy() {
 ...
 GetCharacterMovement()->MaxWalkSpeed=210; GetCharacterMovement()->bOrientRotationToMovement=true;
...
float ASliceEnemy::TakeDamage(float Damage,const FDamageEvent& Event,AController* Instigator,AActor* Causer) {
 ...
 if(Health==0) {
  ...
  auto* M=GetGameInstance()->GetSubsystem<USliceMission>(); M->State.pursuitStarted=true; M->Apply(Wroclaw::Event::LosePursuit);
 }
 return Damage;
}
...
FString ASliceEnemy::StatusText() const {
 ...
 const TCHAR* Labels[]={TEXT("PATROL"),TEXT("PODEJRZENIE"),TEXT("POŚCIG — ZERWIJ KONTAKT"),TEXT("ATAK"),TEXT("ZGUBIŁ CIĘ — UKRYJ SIĘ"),TEXT("PRZESZUKIWANIE OKOLICY"),TEXT("POŚCIG ZAKOŃCZONY"),TEXT("NAPASTNIK POKONANY")};
...
void ASliceEnemyController::Perceived(AActor* Actor,FAIStimulus Stimulus) {
 ...
 if(Stimulus.Type==UAISense::GetSenseID<UAISense_Sight>()) {
  ...
  if(bSees){ LastKnown=Player->GetActorLocation();LastSeen=GetWorld()->GetTimeSeconds();Transition(EEnemyState::Chase);Player->Mission()->State.pursuitStarted=true; }
...
void ASliceEnemyController::Tick(float Dt) {
 ...
 case EEnemyState::Search:
  if(Now-StateSince>9 && FVector::Dist(Enemy->GetActorLocation(),LastKnown)<170) {
   Player->Mission()->Apply(Wroclaw::Event::LosePursuit);Transition(EEnemyState::ReturnToPatrol);
  } else if(Now-StateSince>16) {Player->Mission()->Apply(Wroclaw::Event::LosePursuit);Transition(EEnemyState::ReturnToPatrol);}
  break;
 ...
 case EEnemyState::Patrol: {
  static const FVector Points[]={{3250,2450,95},{4350,2800,95},{3300,3000,95}};
  Goal=Points[PatrolIndex]; if(FVector::Dist2D(Enemy->GetActorLocation(),Goal)<110) PatrolIndex=(PatrolIndex+1)%3;
  break;
 }
 default:break;
 }
